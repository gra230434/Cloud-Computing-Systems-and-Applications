...
class ControllerThread(threading.Thread):
    ...
    def __init__(self, interval, ki=None, kp=None):
        ...
        self.cpuPeriod = 0.03
        if ki is None:
            self.ki = 0.2  # -0.005;   # integral constant of th PI regulator
        if kp is None:
            self.kp = 0.02  # 0.05 proportional constant of th PI regulator
    ...
    def run(self):
        while self.running:
            ...
            if self.cpuPeriod < 0:  # anti wind up control
                self.cpuPeriod = 0
